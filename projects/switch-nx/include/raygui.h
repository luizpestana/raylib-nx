...
//----------------------------------------------------------------------------------
// LibNx Specific Functions Definition (PLATFORM_NX)
//----------------------------------------------------------------------------------

// Show nx virtual keyboard
int ShowNxKeyboard(char* text, int textSize, const char* title, const char* placehold, const char* ok, const char* initial, SwkbdType type)
{
    SwkbdConfig kbd;
...
int GuiTextBox(Rectangle bounds, char *text, int textSize, bool editMode)
{
        ...
        (wrapMode == TEXT_WRAP_NONE))               // No wrap mode
    {
        ...
        if (editMode)
        {
            ...
            result = ShowNxKeyboard(text, textSize, "", "", "Enter", text, SwkbdType_QWERTY);
...
int GuiValueBox(Rectangle bounds, const char *text, int *value, int minValue, int maxValue, bool editMode)
{
    ...
    if ((state != STATE_DISABLED) && !guiLocked && !guiControlExclusiveMode)
    {
        ...
        if (editMode)
        {
            ...
            result = ShowNxKeyboard(textValue, RAYGUI_VALUEBOX_MAX_CHARS, "", "", "Enter", textValue, SwkbdType_NumPad);
            valueHasChanged = (result == 1);
